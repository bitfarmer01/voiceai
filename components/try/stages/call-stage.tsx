...
import type { Booking, BudgetState, KnowledgeChunk, ServiceDetail } from "@/lib/types";
...
import { ReferencePanel } from "@/components/try/reference-panel";
 ...
 * your-business calls. Left column: voice + transcript + booking card + details
 * toggle. Right column: persistent reference panel (services, hours, knowledge
 * source with live highlights). Single column below lg breakpoint.
 */
export function CallStage({
  ...
  return (
    <div className="px-4 py-6">
      ...
      {/* Two-column grid: call card left, reference panel right */}
      <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
        {/* Left column: call card + details toggle */}
        <div className="flex flex-col gap-4">
          <div className="flex flex-col overflow-hidden rounded-2xl border bg-card">
            ...
            <div className="border-t p-5">
              ...
              {startError && (
                <p className="mt-3 text-center text-xs text-danger">{startError}</p>
              )}
            ...
            <div className="border-t p-4">
              {call.turns.length === 0 ? (
                <EmptyState
                  title={
                    call.status === "idle" ? "The conversation shows up here" : "Listening…"
                  }
                  description={
                    call.status === "idle"
                      ? "Press the call button to start the conversation."
                      : undefined
                  }
                />
              ) : (
